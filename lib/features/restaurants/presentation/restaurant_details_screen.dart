...
final _restaurantProvider = FutureProvider.autoDispose
    .family<Restaurant, String>((ref, id) =>
        ref.watch(restaurantRepositoryProvider).fetchRestaurant(id));

final _summaryProvider = FutureProvider.autoDispose
    .family<RestaurantSummary, String>((ref, id) =>
        ref.watch(restaurantRepositoryProvider).fetchSummary(id));
...
class RestaurantDetailsScreen extends ConsumerWidget {
  ...
  Future<void> _openDirections(
      BuildContext context, WidgetRef ref, Restaurant r) async {
    unawaited(ref
        .read(analyticsProvider)
        .directionsOpened(restaurantId: restaurantId));
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final saved =
        (ref.watch(savedIdsProvider).value ?? {}).contains(restaurantId);
    final visited =
        (ref.watch(visitedIdsProvider).value ?? {}).contains(restaurantId);
      ...
      body: restaurant.when(
        ...
        data: (r) => CustomScrollView(
          slivers: [
            SliverAppBar(
                ...
                IconButton(
                  ...
                  onPressed: () {
                    unawaited(ref.read(analyticsProvider).shareInitiated(
                        contentType: 'restaurant', id: restaurantId));
                    unawaited(SharePlus.instance.share(ShareParams(
                        text:
                            'Check out ${r.name} on WanderBites')));
                  },
                ),
              ...
              flexibleSpace: FlexibleSpaceBar(
                background: r.coverPhotoUrl == null
                    ...
                    : CachedNetworkImage(
                        imageUrl: r.coverPhotoUrl!, fit: BoxFit.cover),
              ),
            ),
            SliverToBoxAdapter(
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _CuisineRow(restaurantId: restaurantId, price: r.priceLevel),
                    ...
                    Text('Recommended by people you trust',
                        style: theme.textTheme.titleMedium
                            ?.copyWith(fontWeight: FontWeight.w700)),
                    const SizedBox(height: WbSpacing.sm),
                    ref.watch(_summaryProvider(restaurantId)).when(
                          loading: () => const WbSkeleton(height: 36),
                          error: (_, _) => const SizedBox.shrink(),
                          data: (s) => s.tasters.isEmpty
                              ? Text('No recommendations yet. Be the first.',
            ...
            ref.watch(_recsProvider(restaurantId)).when(
  ...
  Future<void> _report(BuildContext context, WidgetRef ref) async {
    ...
    await ref.read(restaurantRepositoryProvider).reportRestaurant(
        reporterId: session.user.id, restaurantId: restaurantId);
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
